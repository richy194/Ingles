    ...
    <div class="page-shell">
        <section class="hero">
            <div>
                ...
                <p>Actualiza la información de {{ $formulario->name }}.</p>
            </div>
            <div class="actions">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('formularios.update', $formulario->id) }}" method="POST">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="estado" name="estado">
                                ...
                                <option value="aprobado" {{ old('estado', $formulario->estado) == 'aprobado' ? 'selected' : '' }}>Aprobado</option>
                                <option value="desaprobado" {{ old('estado', $formulario->estado) == 'desaprobado' ? 'selected' : '' }}>Desaprobado</option>
                                <option value="cancelado" {{ old('estado', $formulario->estado) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                                <option value="no aprobado" {{ old('estado', $formulario->estado) == 'no aprobado' ? 'selected' : '' }}>No aprobado</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="grupo_id" name="grupo_id" class="form-control" required>
                                ...
                                @foreach ($cursos as $curso)
                                    <option value="{{ $curso->id }}" {{ old('grupo_id', $formulario->grupo_id) == $curso->id ? 'selected' : '' }}>{{ $curso->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="teacher_id" name="teacher_id" required>
                                ...
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $formulario->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        <button type="submit" class="btn btn-brand">Actualizar Inscripción</button>
                        <a href="{{ route('formularios.index') }}" class="btn btn-neutral">Cancelar</a>
                    </div>
                </form>
            </div>
